...
public class ArticleController {
    ...
    private Stream<Article> getStreamArticlesForUser(String username) {
        return articles.values().stream().filter(article -> article.getOwner().getUsername().equals(username));
    }
    ...
    public List<Article> getArticlesAvailableForUser(String username) {
        return getStreamArticlesForUser(username).filter(Article::isAvailable).toList();
    }
    ...
    public List<Article> getArticlesEditableForUser(String username) {
        return getStreamArticlesForUser(username).filter(Article::isEditable).toList();
    }
    ...
    public List<Article> getArticlesForCategory(String categoryRootName, String categoryLeafName, boolean isAdmin) {
        ...
        return articles.values().stream().filter(article -> article.getCategory().equals(category)).filter(article -> {
            ArticleState state = article.getState();
            if (isAdmin) {
                return state.equals(ArticleState.OFFERTA_APERTA) || state.equals(ArticleState.OFFERTA_SCAMBIO) || state.equals(ArticleState.OFFERTA_CHIUSA);
            } else {
    ...
    public List<Article> getAvailableArticlesForExchange(User user, Category category) {
        ...
        return articles.values().stream().filter(article -> article.getCategory().equals(category) && article.isAvailable() && !article.getOwner().equals(user)).toList();
    }
}
